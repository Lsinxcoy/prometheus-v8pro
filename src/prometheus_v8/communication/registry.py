...
class AgentRegistry:
    ...
    def find_by_capability(self, capability: str) -> list[AgentInfo]:
        with self._lock:
            return [
                a for a in self._agents.values() if capability in a.capabilities and a.is_alive(self._heartbeat_timeout)
            ]
        ...
        with self._lock:
            for agent_id, info in list(self._agents.items()):
                if info.status != "dead" and not info.is_alive(self._heartbeat_timeout):
                    info.status = "dead"
                    logger.warning(
                        f"Agent zombie detected: {agent_id}, last heartbeat {time.time() - info.last_heartbeat:.0f}s ago"
                    )
