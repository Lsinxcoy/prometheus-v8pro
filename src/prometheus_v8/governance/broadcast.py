...
class BroadcastManager:
    ...
    def send(
        ...
        if self._bus:
            ...
            self._bus.publish(
                Message(
                    channel=f"broadcast_{level.value}",
                    sender=sender,
                    type=level.value,
                    payload={"broadcast_id": bc.id, "content": content},
                )
            )
    ...
    def get_active(self, level: BroadcastLevel | None = None) -> list[Broadcast]:
        ...
        with self._lock:
            bcs = [
                b for b in self._broadcasts.values() if not b.rolled_back and (b.expires_at == 0 or b.expires_at > now)
            ]
            if level:
    ...
    def _request_approval(self, bc: Broadcast) -> bool:
        for cb in self._approval_callbacks:
            try:
                ...
                logger.debug(f"Broadcast approval callback error: {e}")
                pass
        # Auto-approve L0-L2
