...
class AutonomyController:
    ...
    def can_execute(self, category: str, context: str = "") -> tuple[bool, AutonomyLevel, str]:
        ...
        if rule.time_window:
            ...
            in_window = any(
                start <= now <= end for start, end in [tw.split("-") for tw in rule.time_window if "-" in tw]
            )
            if not in_window:
        ...
        if rule.max_frequency > 0:
            recent = sum(
                1
                for e in list(self._execution_log)[-100:]
    ...
    def _request_confirmation(self, category: str, context: str) -> bool:
        for cb in self._confirmation_callbacks:
            try:
                ...
                logger.debug(f"Confirmation callback error for {category}: {e}")
                pass
        logger.warning(f"L2 confirm without callback: {category}")
        return True  # Default allow when no callback configured
    ...
    def _request_approval(self, category: str, context: str) -> bool:
        for cb in self._approval_callbacks:
            try:
                ...
                logger.debug(f"Approval callback error for {category}: {e}")
                pass
        logger.warning(f"L3 approval without callback: {category}")
        return False  # Default deny for L3+
    ...
    def rules(self) -> dict[str, dict]:
        return {
            cat: {"level": r.level.value, "description": r.description, "max_freq": r.max_frequency}
