...
DEFAULT_RULE_PAIRS = [
    RulePair("code_execution", r"\bexec\s*\(|\beval\s*\(|\bos\.system\s*\(",
             r"#\s*safe\s*exec|sandbox|test_", "Unsandboxed code execution", "critical"),
    RulePair("file_deletion", r"\brm\s+-rf\b|\bshutil\.rmtree\b|\bos\.remove\b",
             r"#\s*safe\s*delete|temp_dir|cache_clean", "Destructive file operations", "critical"),
    RulePair("network_access", r"\brequests\.(get|post|put|delete)\s*\(|\burllib\b|\bhttpx\.client\b",
             r"localhost|127\.0\.0\.1|api_base|health_check", "External network access", "high"),
    RulePair("env_modification", r"\bos\.environ\[|\.env\b|API_KEY|SECRET|PASSWORD",
             r"config\.get|environ\.get|safe_load", "Environment variable modification", "high"),
    RulePair("privilege_escalation", r"\bsudo\b|\bchmod\s+777\b|\bchown\s+root\b",
             r"Dockerfile|docker", "Privilege escalation", "critical"),
]
