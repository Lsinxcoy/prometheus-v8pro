...
class HermesLLMAdapter:
            ...
            for attempt in range(self._config.max_retries):
                try:
                    ...
                    if attempt < self._config.max_retries - 1:
                        ...
                        logger.warning(f"LLM request failed (attempt {attempt+1}/{self._config.max_retries}): {e}. Retrying in {delay}s")
                        time.sleep(delay)
                    else:
    ...
    def _do_request(self, messages: list[dict], model: str,
        ...
        self._request_log.append({
            "model": model, "latency": round(latency, 3),
