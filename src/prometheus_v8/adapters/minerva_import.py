...
from prometheus_v8.schema import Node, NodeType, MemoryLayer, Provenance, ProvenanceType, create_fact_node
...
class MinervaImportAdapter:
    ...
    def _convert_item(self, item: dict) -> Node | None:
        ...
        node = create_fact_node(content=content[:2000],
                               importance=float(item.get("importance", 0.5)),
                               tags=item.get("tags", []))
        node.type = node_type
        node.provenance = Provenance(source=ProvenanceType.IMPORTED,
