...
class VersionedResource:
    ...
    def __init__(self, name: str, resource_type: str, initial_content: Any = None) -> None:
        ...
        if initial_content is not None:
            self._add_version(
                content=initial_content, author="init", reason="Initial version", state=ResourceState.COMMITTED
            )

    def propose(self, content: Any, author: str = "", reason: str = "") -> ResourceVersion:
        """Propose a new version."""
        return self._add_version(content=content, author=author, reason=reason, state=ResourceState.PROPOSED)
