...
from prometheus_v8.schema import Genome, FitnessResult, NodeType
...
class EvolutionLayer(ABC):
    ...
    def stats(self) -> dict[str, Any]:
        return {"layer": self.layer_id, "name": self.name, "executions": self._execution_count,
...
class L0MetaParams(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        return EvolutionResult(layer=0, layer_name="meta_params", success=True,
                              fitness_delta=delta, output=new_config, time_elapsed=time.time() - start)
...
class L1Strategy(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        best_dir = max(self.DIRECTIONS, key=lambda d: (
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=1, layer_name="strategy", success=True,
                              fitness_delta=delta, output={"direction": best_dir}, time_elapsed=time.time() - start)
...
class L2Skill(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=2, layer_name="skill", success=acquired is not None,
                              fitness_delta=delta, output={"acquired": acquired}, time_elapsed=time.time() - start)
...
class L3Config(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=3, layer_name="config", success=delta > 0,
                              fitness_delta=delta, output={"benchmark_score": benchmark_score}, time_elapsed=time.time() - start)
...
class L4Code(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=4, layer_name="code", success=mutated_code != genome.code,
                              fitness_delta=delta, output={"mutation": mutation_type}, time_elapsed=time.time() - start)
...
class L5MetaEvolution(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=5, layer_name="meta_evolution", success=is_stagnant,
                              fitness_delta=delta, output={"stagnant": is_stagnant}, time_elapsed=time.time() - start)
...
class L6Prompt(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=6, layer_name="prompt", success=optimized != original,
                              fitness_delta=delta, output={"optimized_prompt_idx": prompt_idx}, time_elapsed=time.time() - start)
...
Return ONLY the improved prompt."""
                return self._llm.complete([{"role": "user", "content": rewrite_prompt}], temperature=0.5, max_tokens=500)
            except Exception:
                pass
...
class L7Tool(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=7, layer_name="tool", success=True,
                              fitness_delta=delta, output={"recommended": genome.tools}, time_elapsed=time.time() - start)
...
class L8Memory(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        best_strategy = max(self._strategies, key=lambda s: sum(self._strategy_scores[s][-3:]) / len(self._strategy_scores[s][-3:]))
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=8, layer_name="memory", success=True,
                              fitness_delta=delta, output={"strategy": best_strategy}, time_elapsed=time.time() - start)
...
class L9Knowledge(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=9, layer_name="knowledge", success=filled is not None,
                              fitness_delta=delta, output={"filled_gap": filled, "total_gaps": len(self._knowledge_gaps)},
...
class L10Collaboration(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=10, layer_name="collaboration", success=True,
                              fitness_delta=delta, output={"optimal_team_size": self._optimal_team_size},
                              time_elapsed=time.time() - start)
...
class L11Architecture(EvolutionLayer):
    ...
    def execute(self, ctx: EvolutionContext, genome: Genome, **kwargs) -> EvolutionResult:
        ...
        delta = genome.fitness - old_fitness
        self._execution_count += 1
        return EvolutionResult(layer=11, layer_name="architecture", success=True,
                              fitness_delta=delta, output={"health": health, "repairs": repairs},
                              time_elapsed=time.time() - start)
    ...
    def _repair(self, metric: str, value: float, genome: Genome) -> str | None:
        ...
        if action and metric == "complexity" and len(genome.code) > 5000:
            genome.code = genome.code[:5000]  # Truncate as emergency repair
        return action
...
class UnifiedEvolutionEngine:
    ...
    def __init__(self, llm=None, fitness_evaluator=None, embedder=None) -> None:
        self._llm = llm
    ...
    def evolve(self, genome: Genome, max_generations: int = 100, 
               fitness_threshold: float = 0.95, max_stagnation: int = 20) -> Genome:
        ...
        for gen in range(max_generations):
            ...
            for layer in self._layers:
                ...
                self._history.append({
                    "generation": gen, "layer": layer.layer_id, "name": layer.name,
                    "success": result.success, "fitness_delta": result.fitness_delta,
                })
            ...
            # Track stagnation
            if genome.fitness - old_fitness < 0.001:
                stagnation += 1
            else:
                stagnation = 0
