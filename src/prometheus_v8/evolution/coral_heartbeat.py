...
class CORALHeartbeat:
    ...
    def __init__(self, consolidation_interval: int = 5, stagnation_threshold: int = 10,
                 notes_dir: str = "data/coral_notes") -> None:
    ...
    def reflect(self, task: str, outcome: str, insights: list[str] | None = None,
                mistakes: list[str] | None = None, improvements: list[str] | None = None) -> ReflectionNote:
        """Heartbeat 1: Write a reflection note after task completion."""
        note = ReflectionNote(
            task=task, outcome=outcome,
            insights=insights or [], mistakes=mistakes or [],
    ...
    def _persist_note(self, note: ReflectionNote) -> None:
        ...
        try:
            ...
            path.write_text(json.dumps({
                "task": note.task, "outcome": note.outcome,
                "insights": note.insights, "mistakes": note.mistakes,
                "improvements": note.improvements, "timestamp": note.timestamp,
            }, ensure_ascii=False))
    ...
    def stats(self) -> dict[str, Any]:
        return {
            "tasks": self._task_count, "notes": len(self._notes),
            "skills": len(self._skills), "last_consolidation": self._last_consolidation,
            "fitness_history_len": len(self._fitness_history),
        }
