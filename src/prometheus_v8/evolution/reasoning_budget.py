...
class ReasoningBudget:
    ...
    def __init__(
        self, tokens: int = 4000, time_seconds: int = 240, max_steps: int = 50, initial_entropy: float = 1.0
    ) -> None:
        self._state = BudgetState(
            tokens_limit=tokens,
            time_limit=float(time_seconds),
            max_steps=max_steps,
    ...
    def reset(self) -> None:
        self._state = BudgetState(
            tokens_limit=self._state.tokens_limit, time_limit=self._state.time_limit, max_steps=self._state.max_steps
        )
        self._start_time = time.time()
