...
from prometheus_v8.schema import Node, MemoryLayer, compute_weibull_retention
...
class MetabolismEngine:
    ...
    def triage(self, node: Node) -> TriageResult:
        ...
        if gravity > 0.7 and node.access_count > 10:
            return TriageResult(node_id=node.id, decision=TriageDecision.PROMOTE,
                              reason=f"high gravity ({gravity:.3f}), frequent access", score=gravity,
                              target_layer=self._next_layer(node.layer))
        
        if gravity > 0.3:
            return TriageResult(node_id=node.id, decision=TriageDecision.KEEP,
                              reason=f"adequate gravity ({gravity:.3f})", score=gravity)
        
        if gravity > 0.1:
            return TriageResult(node_id=node.id, decision=TriageDecision.DECAY,
                              reason=f"low gravity ({gravity:.3f})", score=gravity)
        
        if node.age_days > 90 and node.access_count < 2:
            return TriageResult(node_id=node.id, decision=TriageDecision.DELETE,
                              reason=f"neglected ({gravity:.3f}), old ({node.age_days:.0f}d)", score=gravity)
        
        if node.age_days > 30 and node.importance > 0.5:
            return TriageResult(node_id=node.id, decision=TriageDecision.ARCHIVE,
                              reason=f"valuable but old ({node.age_days:.0f}d)", score=gravity,
                              target_layer=MemoryLayer.ARCHIVE)
        
        return TriageResult(node_id=node.id, decision=TriageDecision.DECAY,
                          reason=f"very low gravity ({gravity:.3f})", score=gravity)
    ...
    def run_metabolism_cycle(self, nodes: list[Node]) -> dict[str, int]:
        ...
        for node in nodes:
            ...
            self._audit_log.append({
                "node_id": node.id.hex(), "decision": triage_result.decision.value,
                "gravity": triage_result.score, "reason": triage_result.reason,
                "timestamp": time.time(),
            })
