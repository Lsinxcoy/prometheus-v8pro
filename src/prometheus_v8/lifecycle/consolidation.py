...
from prometheus_v8.schema import Node, MemoryLayer, NodeType, Edge, EdgeType, Provenance, ProvenanceType
...
class ConsolidationPipeline:
    ...
    def _promote(self, node: Node, next_layer: MemoryLayer) -> Node | None:
        ...
        node.layer = next_layer
        node.updated_at = time.time()
        node.decay_hits()  # Reset consecutive hits after promotion
        node.provenance = Provenance(source=ProvenanceType.CONSOLIDATION, confidence=0.8)
