...
def create_cli() -> Any:
    ...
    @app.command()
    def add(content: str = typer.Argument(..., help="Node content"),
            node_type: str = typer.Option("fact", help="Node type (fact/insight/episode/mutation/dream/curiosity/learning)"),
        ...
        creators = {
            "fact": create_fact_node, "insight": create_insight_node,
            "episode": create_episode_node, "mutation": create_mutation_node,
            "dream": create_dream_node, "curiosity": create_curiosity_node,
    ...
    @app.command()
    def get(node_id: str = typer.Argument(..., help="Node ID (hex)"),
            db_path: str = typer.Option("data/prometheus_v8.db", help="Database path")):
        """Get a node by ID."""
        from prometheus_v8.core.store import SQLiteStore
        store = SQLiteStore(db_path)
        ...
        typer.echo(json.dumps({
            "id": node.id.hex(), "type": node.type.value, "content": node.payload.content,
    ...
    @app.command()
    def delete(node_id: str = typer.Argument(..., help="Node ID (hex)"),
               db_path: str = typer.Option("data/prometheus_v8.db", help="Database path")):
        """Delete a node by ID."""
        from prometheus_v8.core.store import SQLiteStore
        store = SQLiteStore(db_path)
    ...
    @app.command()
    def evolve(code: str = typer.Argument(..., help="Code to evolve"),
    ...
    @app.command()
    def consolidate(db_path: str = typer.Option("data/prometheus_v8.db", help="Database path"),
                    limit: int = typer.Option(100, help="Max nodes to process")):
        """Run memory consolidation."""
        from prometheus_v8.core.store import SQLiteStore
        from prometheus_v8.lifecycle.consolidation import ConsolidationEngine
        store = SQLiteStore(db_path)
        engine = ConsolidationEngine(store=store)
        result = engine.run(limit=limit)
        store.close()
        typer.echo(f"Consolidation complete: {result}")

    @app.command()
    def dream(topic: str = typer.Option("", help="Dream topic"),
              db_path: str = typer.Option("data/prometheus_v8.db", help="Database path")):
        """Run a dream cycle."""
        from prometheus_v8.core.store import SQLiteStore
        from prometheus_v8.lifecycle.dream import DreamEngine
        store = SQLiteStore(db_path)
        engine = DreamEngine(store=store)
        result = engine.run(topic=topic)
        store.close()
        typer.echo(f"Dream cycle complete: {result}")
