...
class DashboardProvider:
    ...
    def __init__(
        self, store=None, engine=None, safety=None, monitor=None, governance=None, agents=None, knowledge=None
    ) -> None:
        self._store = store
        self._engine = engine
        self._safety = safety
        ...
        if self._store:
            ...
            for ntype in NodeType:
                try:
                    ...
                    logger.debug(f"Node type distribution query failed for {ntype.value}: {e}")
                    pass
            ...
            if total > 0:
                # Query actual trust distribution from store
                try:
                    verified_count = 0
                    high_signal_count = 0
                    pending_count = 0
                    # Count by trust_level via SQL
                    conn = self._store._get_conn()
                    trust_rows = conn.execute(
                        "SELECT trust_level, COUNT(*) as cnt FROM nodes GROUP BY trust_level"
                    ).fetchall()
                    for row in trust_rows:
                        tl = row[0] if isinstance(row[0], str) else row["trust_level"]
                        cnt = row[1] if isinstance(row[1], int) else row["cnt"]
                        if tl == "verified":
                            verified_count = cnt
                        elif tl == "high_signal":
                            high_signal_count = cnt
                        elif tl == "pending":
                            pending_count = cnt
                    distribution["by_trust"]["verified_ratio"] = verified_count / total
                    distribution["by_trust"]["high_signal_ratio"] = high_signal_count / total
                    distribution["by_trust"]["pending_ratio"] = pending_count / total
                except Exception as e:
                    logger.debug(f"Trust distribution query failed: {e}")
    ...
    def get_key_metrics(self) -> list[DashboardMetric]:
        ...
        metrics.append(
            DashboardMetric(
                name="Safety Checks",
                value=safety_stats.get("checks", 0),
                unit="checks",
                trend="stable",
                status="normal",
            )
