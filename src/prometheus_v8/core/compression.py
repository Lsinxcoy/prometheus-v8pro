...
class INT8Compressor:
    ...
    def compress(self, vector: list[float] | np.ndarray) -> tuple[bytes, float]:
        """Quantize float32 vector to int8. Returns (packed_bytes, scale_factor)."""
        vec = np.asarray(vector, dtype=np.float32)
        max_val = np.max(np.abs(vec)) or 1.0
        scale = max_val / 127.0
        quantized = np.clip(np.round(vec / scale), -128, 127).astype(np.int8)
        return struct.pack(f"{len(quantized)}b", *quantized), scale

    def decompress(self, data: bytes, scale: float = 1.0) -> list[float]:
        """Dequantize int8 back to float32 using scale factor."""
        count = len(data)
    ...
    def compress_with_scale(self, vector: list[float] | np.ndarray) -> tuple[bytes, float]:
        ...
        Alias for compress() which now always returns scale.
        """
        return self.compress(vector)

    def decompress_with_scale(self, data: bytes, scale: float) -> list[float]:
        """Dequantize using stored scale factor.

        Alias for decompress() with explicit scale.
        """
        return self.decompress(data, scale)
