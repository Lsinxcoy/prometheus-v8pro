...
from prometheus_v8.schema import (
    Node, Edge, NodePayload, WeibullParams, Provenance,
    NodeType, EdgeType, MemoryLayer, MemoryScope,
    Veracity, TrustLevel, generate_uuidv7, compute_checksum,
)
...
class SQLiteStore:
    ...
    def __init__(self, db_path: str = "data/prometheus_v8.db",
                 cache_size_mb: int = 64, mmap_size_mb: int = 512) -> None:
    ...
    def add_node(self, node: Node) -> bytes:
        with self._transaction() as conn:
            ...
            conn.execute(
                ...
                (node.id, node.type.value, node.layer.value, node.scope.value,
                 node.payload.content, emb, node.payload.checksum,
                 node.importance, node.confidence, node.veracity.value, node.trust_level.value,
                 node.access_count, node.consecutive_hits,
                 node.weibull.lam, node.weibull.k,
                 node.provenance.source.value, node.provenance.agent_id, node.provenance.confidence,
                 json.dumps(node.tags), json.dumps(node.metadata), node.branch,
                 node.created_at, node.updated_at, node.accessed_at,
                 node.valid_from, node.valid_to,
                )
            )
            # FTS5 insert - use auto-increment rowid
            conn.execute("INSERT INTO nodes_fts(content,tags) VALUES(?,?)",
                        (node.payload.content, " ".join(node.tags)))
        return node.id
    ...
    def get_node(self, node_id: bytes) -> Optional[Node]:
        ...
        node.touch()
        conn.execute("UPDATE nodes SET access_count=?, consecutive_hits=?, accessed_at=?, updated_at=? WHERE id=?",
    ...
    def update_node(self, node: Node) -> None:
        ...
        with self._transaction() as conn:
            conn.execute("INSERT INTO node_versions(node_id,content,importance,confidence,timestamp) VALUES(?,?,?,?,?)",
    ...
    def search_fts(self, query: str, limit: int = 20) -> list[Node]:
        conn = self._get_conn()
        # Simple FTS5 search - get matching content, then look up in nodes table
        fts_rows = conn.execute(
            "SELECT content FROM nodes_fts WHERE nodes_fts MATCH ? ORDER BY rank LIMIT ?",
            (query, limit)
        ).fetchall()
        if not fts_rows:
            return []
        # Match FTS content back to nodes
        results = []
        for row in fts_rows:
            content = row["content"] if hasattr(row, "keys") else row[0]
            node_rows = conn.execute(
                "SELECT * FROM nodes WHERE content=? ORDER BY importance DESC LIMIT 1",
                (content,)
            ).fetchall()
            for nr in node_rows:
                try:
    ...
    def get_nodes_by_type(self, type_: NodeType, limit: int = 100) -> list[Node]:
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM nodes WHERE type=? ORDER BY importance DESC LIMIT ?", (type_.value, limit)).fetchall()
        return [self._row_to_node(r) for r in rows]

    def get_nodes_by_layer(self, layer: MemoryLayer, limit: int = 100) -> list[Node]:
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM nodes WHERE layer=? ORDER BY importance DESC LIMIT ?", (layer.value, limit)).fetchall()
        return [self._row_to_node(r) for r in rows]
    ...
    def get_node_versions(self, node_id: bytes, limit: int = 10) -> list[dict]:
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM node_versions WHERE node_id=? ORDER BY timestamp DESC LIMIT ?", (node_id, limit)).fetchall()
        return [{"version_id": r[0], "content": r[2], "importance": r[3], "confidence": r[4], "timestamp": r[5]} for r in rows]
