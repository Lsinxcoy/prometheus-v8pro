...
class Embedder:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2", dimension: int = 384, device: str = "cpu") -> None:
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        if self._use_st and self._model:
            try:
                ...
                logger.warning(f"ST batch embed error: {e}")
        return [self._hash_embed(t) for t in texts]

    @staticmethod
    def hash_embed(text: str, dimension: int = 384) -> list[float]:
    ...
    def _hash_embed(self, text: str) -> list[float]:
        """Deterministic hash-based embedding fallback (delegates to hash_embed)."""
        return Embedder.hash_embed(text, self._dimension)
