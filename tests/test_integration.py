    ...
    try:
        ...
        for node in nodes:
            ...
            assert result.decision in [
                ...
                TriageDecision.KEEP,
                TriageDecision.DECAY,
                TriageDecision.DELETE,
...
def test_evolution_engine():
    """Test the unified evolution engine (with timeout to prevent hanging)."""
    engine = UnifiedEvolutionEngine()
    from prometheus_v8.schema import Genome

    genome = Genome(code="def hello():\n    return 'world'", fitness=0.3)

    result = engine.evolve(genome, max_generations=2, fitness_threshold=0.99)
